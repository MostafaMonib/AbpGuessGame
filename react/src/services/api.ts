...
export const GameService = {
  async startGame(): Promise<GameDto> {
    const res = await api.post<GameDto>('/api/app/game/start');
    return res.data;
  },

  async submitGuess(value: number, idempotencyKey?: string): Promise<GuessResultDto> {
    const res = await api.post<GuessResultDto>('/api/app/game/guess', {
      value,
  ...
  async getGuessHistory(gameId: string): Promise<GuessHistoryItemDto[]> {
    const res = await api.get<GuessHistoryItemDto[]>(`/api/app/game/history/${gameId}`);
    return res.data;
  },

  async getActiveGame(): Promise<GameDto | null> {
    try {
      const res = await api.get<GameDto>('/api/app/game/active');
      return res.data;
    } catch {
      return null;
    }
